...
void send_attachments_dart(struct AttachmentsDart attachments_dart) {
    printf("Hello World 2\n");
    printf("attachments length %lld\n", attachments_dart.file_length);
    for (int i = 0; i < attachments_dart.file_length; ++i)
    {
        struct FileAttachmentDart fa = *attachments_dart.file_array[i];
        printf("path: %s\n", fa.file_path);
    }
}
